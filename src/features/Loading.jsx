...
export default function Loading({ icon }) {
    ...
    function updateLoadingText() {
        const idx = loadConfig.idx
        const data = loadConfig.data

        if (data[idx + 1]) {
            loadConfig.idx++
            loadConfig.current += data[loadConfig.idx]
        } else {
            loadConfig.idx = 0
            loadConfig.current = data[loadConfig.idx]
        }
        setText(loadConfig.current)
    }

    return (
        <div
            className="pos-absolute top-0 width-100 left-0"
            style={{
        ...
        >
            <div className="d-flex justify-content-center align-items-center flex-column width-100 height-90">
                {icon === "text" && (
                    <p className="typography animate-text">{text}</p>
                )}
                {icon === "spin" && (
                    <div className="spinner large mt-1">
                        <span className="">
                            <svg
